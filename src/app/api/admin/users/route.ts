...
// Only the parameters GET /admin/users documents are forwarded — anything else
// in the query string is dropped.
const FORWARDED_PARAMS = ["page", "limit", "search", "sort", "order"] as const;
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const backendRes = await fetch(`${apiUrl}/admin/users${query ? `?${query}` : ""}`, {
      method: "GET",
    ...
    console.error("[API/admin/users] Error:", error instanceof Error ? error.message : error);
